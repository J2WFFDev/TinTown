...
if ($shotsDetected.Count -eq 6) {
    ...
    Write-Host "Detected shots at samples: $(($shotsDetected | ForEach-Object { $_.StartSample }) -join ', ')"
...
Write-Host "`nThe bridge is ready to detect shots in real-time with the validated criteria."